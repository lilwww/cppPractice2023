...
Thread::Thread(ThreadCallback &&cb)
    : _thid(0), _isRunning(false), _cb(std::move(cb)) // 注册回调函数
{
}
//default construct
Thread::Thread()
    : _thid(0), _isRunning(false), _cb(nullptr)
{
}
Thread::~Thread()
{
...
void Thread::start()
{
    ...
    // 中没有对象d'd去调用run方法,所以第四个参数需要传递对象进来
...
bool Thread::getRunState()
{
    return _isRunning;
}

void Thread::setCallback(ThreadCallback &&cb)
{
    _cb = cb;
}